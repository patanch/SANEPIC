...
void usage(char *name)
{
  ...
  cerr << "-f <sample list file mane>  file including list of 1st and last samples" << endl;
...
int main(int argc, char *argv[])
{
  ...
  while ( (retval = getopt(argc, argv, "F:Z:f:n:l:d:D:C:H:h:w:J:o:O:B:R:X:G:P:e:p:A:m:k:K:u:c:N:n:L:g:r:M:E:j:z:b:I:i:")) != -1) {
    switch (retval) {
    case 'F':
      dirfile = optarg;
      break;
...
//	  }
  ...
  int iter;
  double var0, var00, var00_, var_n, var_n_, delta0, delta_n, delta_n_, delta_o, rtq, rtqt, alpha, beta, chi2, chi20, chi2tot, absrecal, absrecal_old;
  ...
  while (rloop && rloop < 500 && var0/var00 > StopCriterium){
    ...
    while((iter < 2000 && var_n/var00 > StopCriterium) || iter == 0){
        ...
        //************************************************************  Write variance maps Interation 1 ****************************************************************//
        ...
        if (iter == 1 && newrank == 0 && rloop == 1){
	        ...
	        if (polar){
	            ...
	            if (rank == 0){
	                sprintf(mapoutstr, "%s_Q_NVAR%s", outdir.c_str(),termin_out.c_str());
	                write_reduced_map(mapoutstr,nside,tmpinitmap+npix,indpix);
	                sprintf(mapoutstr, "%s_U_NVAR%s", outdir.c_str(),termin_out.c_str());
	            ...
	            if (nsubm > 1){
	                ...
	                if (rank == 0){
	                    ...
	                    for (ii=0;ii<npix;ii++)
		                    tmptab[ii] = double(tmptabl[ii]);
	                    sprintf(mapoutstr, "%s_POL_HITS%s", outdir.c_str(),termin_out.c_str());
	                    write_reduced_map(mapoutstr,nside,tmptab,indpix);
	                }
	            } else {
                    /// just copy data
	            ...
	            if (GalBP && (rank == 0)){
	                sprintf(mapoutstr, "%s_Leak_NVAR%s", outdir.c_str(),termin_out.c_str());
	        ...
	        delete [] tmptabl;
	        delete [] tmpinitmap;
        } //******************************************************* End of variance writting at iteration 1 **************************************************************//
        ...
        if ((newrank == 0) && iterw && (iter % iterw) == 0 && (rloop == 1)){
            ...
            if (writeIterOn){
                if (rank == 0){
	                sprintf(mapoutstr, "%s_I%s_%d", outdir.c_str(), termin_out.c_str(), iter);
	                write_reduced_map(mapoutstr,nside,tmpinitmap,indpix);
                }

	            if (polar && (rank == 0)){

	                sprintf(mapoutstr, "%s_Q%s_%d", outdir.c_str(),termin_out.c_str(), iter);
	                write_reduced_map(mapoutstr,nside,tmpinitmap+npix,indpix);
	                sprintf(mapoutstr, "%s_U%s_%d", outdir.c_str(),termin_out.c_str(), iter);
	                write_reduced_map(mapoutstr,nside,tmpinitmap+2*npix,indpix);	

	                if (GalBP && (rank == 0)){
	                    sprintf(mapoutstr, "%s_Leak%s_%d", outdir.c_str(),termin_out.c_str(), iter);
	                    write_reduced_map(mapoutstr,nside,tmpinitmap+3*npix,indpix);
	                }
	            } else if (GalBP && (rank == 0)){
	                sprintf(mapoutstr, "%s_Leak%s_%d", outdir.c_str(),termin_out.c_str(), iter);
    ...
    if (newrank == 0){
        ...
        if (rank == 0){
            //// Write map globaly
            sprintf(mapoutstr, "%s_A_%s_%d", outdir.c_str(),termin_out.c_str(),rloop);
            NbWrite= WriteMAP(tmpinitmap, mapoutstr, 8, npixtot);

	        sprintf(mapoutstr, "%s_I_%s_%d", outdir.c_str(),termin_out.c_str(),rloop);
	        write_reduced_map(mapoutstr,nside,tmpinitmap,indpix);

	        if (polar){
	  
	            sprintf(mapoutstr, "%s_Q_%s_%d", outdir.c_str(),termin_out.c_str(),rloop);
	            write_reduced_map(mapoutstr,nside,tmpinitmap+npix,indpix);
	            sprintf(mapoutstr, "%s_U_%s_%d", outdir.c_str(),termin_out.c_str(),rloop);
	            write_reduced_map(mapoutstr,nside,tmpinitmap+2*npix,indpix);	
	  
	            if (GalBP){
	                sprintf(mapoutstr, "%s_Leak_%s_%d", outdir.c_str(),termin_out.c_str(),rloop);
	                write_reduced_map(mapoutstr,nside,tmpinitmap+3*npix,indpix);
	            }
	        } else if (GalBP){
	            sprintf(mapoutstr, "%s_Leak_%s_%d", outdir.c_str(),termin_out.c_str(),rloop);
